---
name: motion-framer
description: Premium, Framer-grade web motion. Covers the Motion / Framer Motion library for React/JS (motion components, variants, gestures hover/tap/drag, layout & AnimatePresence exit animations, spring physics, scroll/whileInView effects) AND the equivalent techniques in plain vanilla HTML/CSS/JS for static single-file landing pages (see references/vanilla-css-motion.md). Use this skill whenever the user wants to add motion, animation, micro-interactions, scroll reveals, hover effects, parallax, an animated or aurora hero, a logo marquee, or a "premium / Framer-like / alive" feel to any UI or landing page, React or not, even if they don't name Framer Motion explicitly.
---
...
> **Not using React?** The `motion.` component API below is React-only. If the
> target is a **static HTML page** (a single-file landing, a page pasted into a
> CMS like GoHighLevel, anything not React), read
> **`references/vanilla-css-motion.md`** instead: it implements the same
> premium-motion principles (spring easing, scroll reveals, stagger, hover
> micro-interactions, parallax, animated/aurora hero, logo marquee, count-up,
> magnetic buttons) in plain CSS + a little JS, keeping the page single-file and
> portable. Single-file landing pages are this case.
...
**references/**
- `api_reference.md` - Complete Motion (React) API reference
- `vanilla-css-motion.md` - The same premium-motion principles in plain HTML/CSS/JS, for static / non-React pages (single-file landings, CMS-pasted HTML). Read this when the target is not a React app.
